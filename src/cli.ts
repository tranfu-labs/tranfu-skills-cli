...
program
  ...
  .option(
    "--scope <scope>",
    "user / project / profile:<name> (profile 仅 hermes; TTY 未传则弹选, 非 TTY 默认 user)"
  )
  .option(
    "--runtime <runtime>",
    "claude-code / codex / hermes (TTY 未传 + 多 runtime 弹选, 非 TTY 走探测; hermes 未传 --scope 走 detectActiveProfile)"
  )
...
program
  ...
  .option("--json", "JSON 输出")
  .option("--scope <scope>", "user / project / profile:<name> (未传则不过滤)")
  .option("--runtime <runtime>", "claude-code / codex / hermes (未传则不过滤)")
...
program
  ...
  .option("--scope <scope>", "user / project / profile:<name> (未传则不过滤, 全列)")
  .option("--runtime <runtime>", "claude-code / codex / hermes (未传则不过滤, 全列)")
  .option("--json", "JSON 输出")
...
program
  ...
  .option("--scope <scope>", "user / project / profile:<name> (未传则从 registry 查全部)")
  .option("--runtime <runtime>", "claude-code / codex / hermes (未传则从 registry 查全部)")
...
program
  ...
  .option(
    "--runtime <runtime>",
    "claude-code / codex / hermes (未传单 runtime 自动探测; 多 runtime + TTY 走交互)"
  )
  .option("--both", "装到所有探到的 runtime (claude-code + codex + hermes)")
  .action(initCommand);

program
  .command("doctor")
  .description("诊断本机环境 (Node 版本 / runtime / PATH / 旧缓存 / hermes-profile)")
  .option("--json", "JSON 输出")
