...
/**
 * tfs catalog: 列远端公司库 index.json 的全部 skill (不分 runtime, 不带 query 过滤).
 * 是 `tfs search` 的 "看全部" 版本.
 *
 * 本地已装查询 → 用 `tfs list` (= `tfs installed`).
 */
export async function catalogCommand(opts: { json?: boolean }): Promise<void> {
  let index;
  try {
