...
async function withRuntimeDirs(
  exists: { claude?: boolean; codex?: boolean }
...
describe("resolveRuntime — auto-detect", () => {
  ...
  it("both dirs → throws runtime_required with --runtime hint", async () => {
...
describe("detectAvailableRuntimes", () => {
  ...
  it("returns both when both exist (order: claude-code, codex)", async () => {
    const { detectAvailableRuntimes } = await withRuntimeDirs({
      claude: true,
      codex: true,
    });
    expect(detectAvailableRuntimes()).toEqual(["claude-code", "codex"]);
  });
});
