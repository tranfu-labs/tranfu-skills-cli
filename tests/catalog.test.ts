...
describe("catalog — 列远端公司库全部 skill", () => {
  it("默认输出: 全部 skill, 按 type 标注", async () => {
    vi.stubGlobal("fetch", mockFetchIndex(mockIndex));
    const { catalogCommand } = await import("../src/commands/catalog.js");
    const stdoutSpy = vi
  ...
  it("--json 完整 schema (含 source_url for external)", async () => {
    ...
    await catalogCommand({ json: true });
  ...
  it("fetch 404 → exit 1 index_not_initialized", async () => {
    ...
    await expect(catalogCommand({})).rejects.toThrow("process.exit called");
